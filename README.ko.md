...
- **모든 주장은 검증됨.** 각 증거는 독립 원본 저장소로의 참조를 들고 있고, 4단계
  검증이 이를 확인 — 실패하면 화면에서 강등. 데모에선 **16개 관계 중 6개가 강등**됨.
...
**16개 관계 중 6개가 강등** — 대부분 `FLOW_DIRECTION_MISMATCH`: Gemma가 순환의
...
- **음성 대조군**: 100% 검출률이 하네스 자체의 결함(놓침을 못 세는 채점 버그)이
  ...
  같은 방향으로 흐르는 다른 실제 거래로 인용을 바꿔치기하는 것. 이런 "쌍둥이
...
시스템과 실시간 백엔드는 설계는 끝났지만 아직 구현 전 — 있는 그대로 적습니다.
...
강등은 결함이 아니라 핵심입니다 — 검증 루프가 제 일을 하는 것.
